...
const sendEmail = async ({ to, subject, html }) => {
  try {
    await transporter.sendMail({
      from: `"Tick" <${process.env.EMAIL_USER}>`,
      to,
      subject,
      html,
    });
...
export const sendAdminOtp = async (email, name, otp) => {
  await sendEmail({
    to: email,
    subject: "Verify your Tick account",
    html: `
      <div style="font-family:sans-serif;max-width:480px;margin:auto">
        <h2 style="color:#7c3aed">Tick</h2>
...
export const sendCoachCredentials = async (email, name, password) => {
  await sendEmail({
    to: email,
    subject: "Your Tick coach account is ready",
    html: `
      <div style="font-family:sans-serif;max-width:480px;margin:auto">
        <h2 style="color:#0891b2">Tick</h2>
        <p>Hi <strong>${name}</strong>,</p>
...
export const sendCoachVerificationOtp = async (email, name, otp) => {
  await sendEmail({
    to: email,
    subject: "Verify your Tick coach account",
    html: `
      <div style="font-family:sans-serif;max-width:480px;margin:auto">
        <h2 style="color:#0891b2">Tick</h2>
        <p>Hi <strong>${name}</strong>,</p>
